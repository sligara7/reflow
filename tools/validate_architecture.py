...
class ArchitectureValidator:
    ...
    def validate_interface_consistency(self) -> List[dict]:
        ...
        for service_name, service in services.items():
            for interface in service.get("interfaces", []):
                ...
                reg_interface = interface_registry["interfaces"][service_name].get(
                    interface["name"]
                )
        ...
        for service in services.values():
            for relationship in service.get("service_resource_relationships", []):
                resource = (
                    ...
                    relationship["resource_performer"],
                )
    ...
    def validate_dependency_cycles(self) -> List[dict]:
        ...
        try:
            ...
            issues.append(
                {"type": "graph_analysis_error", "error": str(e), "severity": "high"}
            )
    ...
    def validate_directory_structure(self) -> List[dict]:
        ...
        for dir_path in self.system_path.rglob("*"):
            if dir_path.is_dir():
                ...
                if any(
                    x in str(dir_path) for x in ["__pycache__", ".git", "interfaces"]
                ):
                    continue
                ...
                if (
                    dir_path.parent == self.system_path
                    and not (dir_path / "service_architecture.json").exists()
                ):
    ...
    def run_all_validations(self) -> dict:
        ...
        # Combine all issues
        all_issues = (
            directory_issues + interface_issues + resource_issues + dependency_issues
        )
    ...
    def update_working_memory(self):
        ...
        self.working_memory["validation_state"] = {
            "interface_consistency": {
                ...
                "status": self.validation_results["checks"]["interface_consistency"][
                    "status"
                ],
                "issues": self.validation_results["checks"]["interface_consistency"][
                    "issues"
                ],
            },
            "resource_isolation": {
                "last_checked": datetime.now().isoformat(),
                "status": self.validation_results["checks"]["resource_isolation"][
                    "status"
                ],
                "issues": self.validation_results["checks"]["resource_isolation"][
                    "issues"
                ],
            },
            "dependency_cycles": {
                "last_checked": datetime.now().isoformat(),
                "status": self.validation_results["checks"]["dependency_cycles"][
                    "status"
                ],
                "issues": self.validation_results["checks"]["dependency_cycles"][
                    "issues"
                ],
            },
        }
