...
class InterfaceContractGenerator:
    ...
    def generate_interface_contract(
        self, provider: str, consumer: str, interface_def: Dict
    ) -> Dict:
        ...
        comm_pattern = interface_def.get(
            "communication_pattern", interface_def.get("type", "synchronous")
        )
        interaction_map = {
        ...
        iface_type = interface_def.get(
            "interface_type", interface_def.get("protocol", "json")
        )
        format_map = {
            "http_endpoint": "json",
            "message": "json",
        ...
        contract = {
            ...
            "contract": {
                "input_specification": {
                    "format": format_type,
                    "schema": interface_def.get(
                        "request_schema", interface_def.get("message_format", {})
                    ),
                ...
                "error_handling": {
                    ...
                    "retry_policy": {
                        "applicable": (
                            "yes" if interaction_type == "asynchronous" else "no"
                        ),
                        "max_retries": 3,
            ...
            "dependencies": {
                ...
                "environmental_dependencies": self._extract_env_dependencies(
                    interface_def
                ),
            },
            "metadata": {
                ...
                "breaking_changes": [],
                "rationale": interface_def.get(
                    "description", f"Interface between {provider} and {consumer}"
                ),
    ...
    def _extract_env_dependencies(self, interface_def: Dict) -> List[str]:
        ...
        protocol = interface_def.get(
            "protocol", interface_def.get("interface_type", "")
        )
    ...
    def generate_all_contracts(self):
        ...
        for pair in interface_pairs:
            contract = self.generate_interface_contract(
                pair["provider"], pair["consumer"], pair["interface_def"]
            )
    ...
    def generate_summary(self):
        ...
        print(
            "🤖 LLM agents can use these contracts for independent component development"
        )
...
def main():
    if len(sys.argv) != 2:
        print(
            "Usage: python3 generate_interface_contracts.py /path/to/systems/<system_name>/"
        )
        sys.exit(1)
