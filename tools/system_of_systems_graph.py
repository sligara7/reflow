...
def build_system_graph(index: Dict[str, str], include_levels: List[str], system_root: str) -> nx.DiGraph:
    ...
    Args:
        ...
        system_root: System root directory for resolving relative paths (e.g., /path/to/systems/dnd_reflow)
    """
    G = nx.DiGraph()
    service_info = {}
    node_levels = {}
    ...
    for service_id, file_path in index.items():
        # Handle relative paths by making them relative to system root
...
if __name__ == "__main__":
    ...
    # Resolve absolute path to index file and determine system root
    index_path = os.path.abspath(args.index)
    index_dir = os.path.dirname(index_path)
    
    # Determine system root: walk up from index directory to find the system root
    # System root contains context/, specs/, services/, docs/ folders
    system_root = index_dir
    while system_root != os.path.dirname(system_root):  # Not at filesystem root
        # Check if this looks like a system root
        has_context = os.path.exists(os.path.join(system_root, 'context'))
        has_specs = os.path.exists(os.path.join(system_root, 'specs'))
        has_services = os.path.exists(os.path.join(system_root, 'services'))
        
        if has_context and has_specs and has_services:
            break  # Found system root
        system_root = os.path.dirname(system_root)
    ...
    if args.mode == 'multi':
        ...
        for viewpoint in viewpoints:
            ...
            G = build_system_graph(index, include_levels=include_levels, system_root=system_root)
